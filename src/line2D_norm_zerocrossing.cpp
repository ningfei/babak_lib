//*********************************************************************
// "line2D_norm_zerocrossing" finds the best fitting line of the points given 
// by x0 an x1 arrays that passes through the origin.
//
// Inputs:
...
void line2D_norm_zerocrossing(double *x0, double *x1, double *w, double *w_const, int n, double *u)
{
   ...
   for(int i=0; i<n; i++)
   {
      a11 += w[i]*w_const[i]*x0[i]*x0[i];
      a12 += w[i]*w_const[i]*x0[i]*x1[i];
      a22 += w[i]*w_const[i]*x1[i]*x1[i];
   }
