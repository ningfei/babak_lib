...
int main(int argc, char **argv)
{
   ...
   if( niftiFilename(prefix, imagefile) == 0 ) exit(0);
   if(opt_v)
   {
...
//   }
   ...
   if(opt_v) printf("Eigenvectors file: %s\n", eigenvecfile);

   if( eigenvecfile[0] != '\0' )
   {
	  char s[512];
      npc=0;
      fp = fopen(eigenvecfile,"r");
	  while( fscanf(fp, "%s", s) != EOF ) npc++;
      fclose(fp);

      npc /= nt;

      if(opt_v) printf("Number of eigenvectors = %d\n", npc);

      eigenvecs = (float *)calloc(nt*npc,sizeof(float));
      fp = fopen(eigenvecfile,"r");
      for(int i=0; i<nt; i++)
      {
         for(int j=0; j<npc; j++)
   ...
   if( paramfile[0] == '\0' )
   {
      printf("Please specify the motion parameters file using -p <filename> option.\n");
      exit(0);
   }

   if(opt_v)
   {
      printf("Motion parameters file: %s\n",paramfile);
   }

   motion_params = (float *)calloc(nt*6,sizeof(float));
   ...
   if( rwfile[0] == '\0' )
   {
      printf("Please specify the reference waveform file using -rw <filename> option.\n");
      exit(0);
   }

   if(opt_v)
   {
      printf("Reference waveform file: %s\n",rwfile);
   }

   // note: in case of gng 6 was 4. Had to be changed in 3 places here from 4 to 6 
   rw = (float *)calloc(nt*6,sizeof(float));
   fp = fopen(rwfile,"r");
   for(int i=0; i<nt; i++)
   {
      for(int j=0; j<6; j++)
   ...
   if( dataFile[0] == '\0' )
   {
      printf("Please specify an output data file using -d <filename> option.\n");
      exit(0);
   }
   ...
   for(int i=0; i<nt; i++)
   {
      ...
      if( eigenvecfile[0] != '\0' )
      {
         for(int j=0; j<npc; j++)
         {
            fprintf(fp,"%7.4f\t",eigenvecs[i*npc + j]);
         }
      }

      for(int j=0; j<6; j++)
      {
         fprintf(fp,"%7.4f\t",motion_params[i*6 + j]);
      }

      for(int j=0; j<6; j++) // changed from 4 to 6 columns
      {
         fprintf(fp,"%8.6f\t",rw[i*6 + j]); // changed from 4 to 6 columns
      }
   ...
   if(F_flag==0)
   {
      ...
      dfmap = (unsigned char *)calloc(nv, sizeof(unsigned char));
   ...
   else
   {
      ...
      df1map = (unsigned char *)calloc(nv, sizeof(unsigned char));
      if(df1map==NULL) memory_allocation_error("df1map");

      df2map = (unsigned char *)calloc(nv, sizeof(unsigned char));
	...
	if(mask[vox]!=0)
	{
		...
		else
		{
			...
			df1map[vox] = (unsigned char)dfn;
			df2map[vox] = (unsigned char)dfd;
